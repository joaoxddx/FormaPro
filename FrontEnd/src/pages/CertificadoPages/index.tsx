...
export const CertificadoPages = () => {
    ...
    useEffect(() => {
        const fetchCertificado = async () => {
            ...
            // Se veio de sala de aula e quer emitir
            if (!certId && params.get('curso')) {
                ...
                if (userJson) {
                    ...
                    try {
                        const postRes = await api.post('/certificados', novoCert);
                        certId = (postRes.data?.ID_Certificado || postRes.data?.id)?.toString();
                    } catch (e) {
            ...
            if (certId) {
                try {
                    const certRes = await api.get(`/certificados/${certId}`);
                    const cert = certRes.data;
                    if (cert) {
                        setCertificado(cert);
                        if (cert.ID_Usuario) {
                            const uRes = await api.get(`/users/${cert.ID_Usuario}`).catch(() => null);
                            if (uRes?.data) {
                        ...
                        if (cert.ID_Curso) {
                            const cRes = await api.get(`/cursos/${cert.ID_Curso}`).catch(() => null);
                            if (cRes?.data) setCurso(cRes.data);
                        }
                    }
                } catch (error) {
