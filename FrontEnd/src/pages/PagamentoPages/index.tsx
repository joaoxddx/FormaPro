...
export const PagamentoPages = () => {
    ...
    useEffect(() => {
        ...
        if (planoId) {
            api.get(`/planos/${planoId}`)
                ...
                .catch(async () => {
                    try {
                        const all = await api.get('/planos');
                        const found = (Array.isArray(all.data) ? all.data : []).find((p: any) => String(p.ID_Plano || p.id) === String(planoId));
                        setPlano(found || null);
                    } catch {
                        setPlano(null);
                    }
    ...
    const handlePagamento = async (e: React.FormEvent) => {
        ...
        try {
            // Criar Assinatura no NestJS
            const assinRes = await api.post('/assinaturas', {
                ID_Usuario: Number(user.ID_Usuario || user.id),
                ID_Plano: Number(plano.ID_Plano || plano.id)
            });
            const assinSalva = assinRes.data;
            const assinId = assinSalva?.ID_Assinatura || assinSalva?.id || 1;

            // Criar Pagamento no NestJS
            await api.post('/pagamentos', {
                ID_Assinatura: Number(assinId),
                ValorPago: Number(plano.Preco),
                MetodoPagamento: metodo,
                Id_Transacao_Gateway: 'TRANS_' + Math.floor(Math.random() * 1000000)
            });

            setLoading(false);
            navigate('/painel_aluno');
        } catch (error) {
            console.error('Erro ao processar assinatura/pagamento:', error);
            setLoading(false);
            navigate('/painel_aluno');
        }
    };
