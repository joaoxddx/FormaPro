...
export const HomePages = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [cursosRes, catRes, planosRes] = await Promise.all([
                    api.get('/cursos').catch(() => ({ data: [] })),
                    api.get('/categoria').catch(() => ({ data: [] })),
                ...
                try {
                    const avsRes = await api.get('/avaliacoes');
                    const avs = Array.isArray(avsRes.data) ? avsRes.data : [];
                    const avsFull = avs.map((av: any) => {
                        const course = cursosData.find((c: any) => c.ID_Curso === av.ID_Curso);
                        return {
                            ...av,
                            NomeAluno: av.usuario?.NomeCompleto || 'Aluno FormaPro',
